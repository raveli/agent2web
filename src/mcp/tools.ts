...
export function registerSiteTools(server: McpServer, ctx: ToolContext): void {
  ...
  server.registerTool(
    ...
    async args => {
      try {
        const result = store.updateFiles(
          args.slug,
  ...
  server.registerTool(
    ...
    async args => {
      try {
        const { total, rows } = store.listSites(args.limit, args.offset);
  ...
  server.registerTool(
    ...
    async args => {
      try {
        const site = store.requireSite(args.slug);
        const files = args.include_files && site.current_version_id ? store.listFiles(site.current_version_id) : [];
        const versions = store.listVersions(site.id, 5);
        const structured = {
  ...
  server.registerTool(
    ...
    async args => {
      try {
        const file = store.readSiteFile(args.slug, args.path, args.version_id, args.max_bytes);
        const isText =
          file.contentType.startsWith('text/') ||
          file.contentType.startsWith('application/json') ||
          file.contentType.startsWith('image/svg');
        const content = isText ? file.data.toString('utf8') : file.data.toString('base64');
        const structured = {
          slug: args.slug,
          path: file.path,
  ...
  server.registerTool(
    ...
    async args => {
      try {
        const site = store.setAccess(args.slug, args.visibility as Visibility, args.password ?? null);
        const note =
  ...
  server.registerTool(
    ...
    async args => {
      try {
        const site = store.rename(args.slug, args.new_slug, args.title);
        return ok(
          args.response_format as ResponseFormat,
  ...
  server.registerTool(
    ...
    async args => {
      try {
        const site = store.setDomain(args.slug, args.domain ?? null);
  ...
  server.registerTool(
    ...
    async args => {
      try {
        const site = store.requireSite(args.slug);
        const versions = store.listVersions(site.id, args.limit);
        return ok(
          args.response_format as ResponseFormat,
  ...
  server.registerTool(
    ...
    async args => {
      try {
        ...
        const site = store.deleteSite(args.slug);
